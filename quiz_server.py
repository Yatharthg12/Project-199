...
def get_random_question_answer(socket):
    random_index = random.randint(0, len(questions)-1)
    random_question = questions[random_index]
    random_answer = answers[random_index]
    socket.send(random_question.encode("utf-8"))
...
def clientThread(client, socket):
    score = 0
...
while True:
    ...
    new_thread = Thread(target=clientThread, args= (client, socket))
    new_thread.start()